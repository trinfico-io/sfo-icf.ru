...
<div class="wrap">
    ...
    echo Nav::widget([
        ...
        'items' => [
            // главная
            [
                'label' => 'Информация для заемщиков',
                'items' =>
                [
                    ['label' => 'Информация для заемщиков Банк «ТРАСТ» (ПАО)', 'url' => ['/site/trust']],
                    ['label' => 'Информация для заемщиков Связной Банк (АО)', 'url' => ['/site/svyaznoy']]
                ]
            ],
            // is #2343
            ($site->id != 1) ? ('') : (
                [
                    'label' => 'Информация для владельцев облигаций',
                    'label' => 'Владельцам облигаций',
                    'options' => ['class' => 'dropdownitem'],
                    'items' => [
                        ['label' => 'Сообщения для владельцев облигаций', 'url' => ['/site/messages-for-investors']],
                        ['label' => 'Бухгалтерская и финансовая отчетность', 'url' => ['/site/accounting-and-financial-statements']],
                        ['label' => 'Эмиссионная документация', 'url' => ['/site/emission']],
                        ['label' => 'Учредительные документы', 'url' => ['/site/constituent']],
                        ['label' => 'Иные документы', 'url' => ['/site/other-documents']],
                        ['label' => 'Информация для ПВО и инвесторов', 'url' => ['/site/pvo-inform']], 
                        ['label' => '', 'options' => ['role' => 'presentation', 'class' => 'divider']],
                        (!Yii::$app->user->isGuest && Yii::$app->user->identity->status == 20) ? (
                            ['label' => 'Управление пользователями', 'url' => ['/users-management/index']]
                            ) : (''),
                        (Yii::$app->user->isGuest || Yii::$app->user->identity->status == 0) ? ('') : (
            ...
            [
                ...
                'items' =>
                [
                    ['label' => 'Публичный', 'url' => ['/site/about']],
                    ['label' => 'Тестовый', 'url' => ['/site/7d11a721']]
                ]
            ],
    ...
    <div class="container">
        ...
        <?php /*= Yii::$app->session->setFlash('warning', 'Warning text'); */ ?>
        <?= Alert::widget() ?>
        <?php 
...
<footer class="footer">
    ...
    ?>
        <p class="pull-left" title="<?=Yii::$app->getRequest()->getUserIP();?>">&copy; ООО &quot;СФО ИНВЕСТКРЕДИТ ФИНАНС&quot; <?= $age ?></p>
